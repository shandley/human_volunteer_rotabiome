...
p.boxplot.phylum.1
# Note: For the figure in the manuscript we manually replaced the asterisks and removed the ns for aesthitic purposes
...
p.gam.phylum <- ggplot(dat.1, aes(x = day, y = Abundance, color = Phylum)) +
  ...
  labs(title = "Figure 4", y = "Relative Abundance") +
...
ggarrange(p.paired.rich, p.paired.sd, nrow = 2)
# Note: Published plot had a few additional manual edits for aesthetic reasons
...
ggarrange(p.rich.rota_boost, p.sd.rota_boost, nrow = 2)
...
# Note: The final manuscript just showed the top 4 most abundant taxa for space reasons. Code for this is in the following chunk
...
# UniFrac
ord.ps1.uni.top4 <- ordinate(ps1.top4, method = "PCoA", distance = "unifrac")

# weighted UniFrac
ord.ps1.wuni.top4 <- ordinate(ps1.top4, method = "PCoA", distance = "wunifrac")
...
```{r ADONIS}
# Note: Many of these calculations were not included in the final manuscript but are included here for reference
...
p.unique.rdp <- ggplot(rlog.all.rdp, aes(x = day, y = Abundance, group = randomization_arm, color = randomization_arm)) +
  geom_jitter(size = 1, alpha = 0.6, width = 0.15) +
  scale_y_log10() +
  theme(legend.position = "NULL") +
