...
# Change day 7 rota boost factor levels to be DeSeq2 worthy
sample_data(ps1)$d7_roto_boost
sample_data(ps1)$d7_roto_boost <- factor(sample_data(ps1)$d7_roto_boost, labels = c("no_boost", "boost"))
sample_data(ps1)$d7_roto_boost
...
class(sample_data(ps1.con)$day)
sample_data(ps1.con)$d7_roto_boost
sample_data(ps1.con)$d7_roto_boost <- factor(sample_data(ps1.con)$d7_roto_boost, labels = c("no_boost", "boost"))
sample_data(ps1.con)$d7_roto_boost

class(sample_data(ps1.narrow)$day)
sample_data(ps1.narrow)$day <- factor(sample_data(ps1.narrow)$day)
class(sample_data(ps1.narrow)$day)
sample_data(ps1.narrow)$d7_roto_boost
sample_data(ps1.narrow)$d7_roto_boost <- factor(sample_data(ps1.narrow)$d7_roto_boost, labels = c("no_boost", "boost"))
sample_data(ps1.narrow)$d7_roto_boost

class(sample_data(ps1.broad)$day)
sample_data(ps1.broad)$day <- factor(sample_data(ps1.broad)$day)
class(sample_data(ps1.broad)$day)
sample_data(ps1.broad)$d7_roto_boost
sample_data(ps1.broad)$d7_roto_boost <- factor(sample_data(ps1.broad)$d7_roto_boost, labels = c("no_boost", "boost"))
sample_data(ps1.broad)$d7_roto_boost
...
**BOOST: CONTROL SAMPLES**
```{r diff-abund-rotaboost-control-LRT}
...
ds.boost.cont.LRT <- phyloseq_to_deseq2(ps1.con, ~d7_roto_boost + day + d7_roto_boost:day)

# Calculate geometric means prior to estimate size factors
geoMeans.ds.boost.cont.LRT <- apply(counts(ds.boost.cont.LRT), 1, gm_mean)
ds.boost.cont.LRT <- estimateSizeFactors(ds.boost.cont.LRT, geoMeans = geoMeans.ds.boost.cont.LRT)

# Run DeSeq2
dds.boost.cont.LRT <- DESeq(ds.boost.cont.LRT, test="LRT", reduced = ~d7_roto_boost + day)

# Tabulate results
res.dds.boost.cont.LRT = results(dds.boost.cont.LRT)
res.dds.boost.cont.LRT$symbol <- mcols(dds.boost.cont.LRT)$symbol
head(res.dds.boost.cont.LRT[order(res.dds.boost.cont.LRT$padj), ], 4)
write.table(res.dds.boost.cont.LRT, file = "./results/deseq_rotaboost_control_LRT.txt", sep = "\t")
mcols(res.dds.boost.cont.LRT)
summary(res.dds.boost.cont.LRT)

# Plot taxa with lowest p-value
fiss.dds.boost.cont.LRT <- plotCounts(dds.boost.cont.LRT, which.min(res.dds.boost.cont.LRT$padj), 
                   intgroup = c("day","d7_roto_boost"), returnData = TRUE)
ggplot(fiss.dds.boost.cont.LRT,
  aes(x = as.numeric(day), y = count, color = d7_roto_boost, group = d7_roto_boost)) + 
...
# Test for taxa which at one or more time points after time 0 showed a d7-rota-boost-specific effect
# Convert phyloseq object to DeSeq2 table
ds.boost.narrow.LRT <- phyloseq_to_deseq2(ps1.narrow, ~d7_roto_boost + day + d7_roto_boost:day)
...
# Run DeSeq2
dds.boost.narrow.LRT <- DESeq(ds.boost.narrow.LRT, test="LRT", reduced = ~d7_roto_boost + day)
...
**BOOST: BROAD SPECTRUM ANTIBIOTICS SAMPLES**
```{r diff-abund-rotaboost-broad-LRT}
# Test for taxa which at one or more time points after time 0 showed a d7-rota-boost-specific effect
# Convert phyloseq object to DeSeq2 table
ds.boost.broad.LRT <- phyloseq_to_deseq2(ps1.broad, ~d7_roto_boost + day + d7_roto_boost:day)

# Calculate geometric means prior to estimate size factors
geoMeans.ds.boost.broad.LRT <- apply(counts(ds.boost.broad.LRT), 1, gm_mean)
ds.boost.broad.LRT <- estimateSizeFactors(ds.boost.broad.LRT, geoMeans = geoMeans.ds.boost.broad.LRT)

# Run DeSeq2
dds.boost.broad.LRT <- DESeq(ds.boost.broad.LRT, test="LRT", reduced = ~d7_roto_boost + day)

# Tabulate results
res.dds.boost.broad.LRT = results(dds.boost.broad.LRT)
res.dds.boost.broad.LRT$symbol <- mcols(dds.boost.broad.LRT)$symbol
head(res.dds.boost.broad.LRT[order(res.dds.boost.broad.LRT$padj), ], 4)
write.table(res.dds.boost.broad.LRT, file = "./results/deseq_rotaboost_braod_LRT.txt", sep = "\t")
mcols(res.dds.boost.broad.LRT)
summary(res.dds.boost.broad.LRT)

# Plot taxa with lowest p-value
fiss.dds.boost.broad.LRT <- plotCounts(dds.boost.broad.LRT, which.min(res.dds.boost.broad.LRT$padj), 
                   intgroup = c("day","d7_roto_boost"), returnData = TRUE)
ggplot(fiss.dds.boost.broad.LRT,
  aes(x = as.numeric(day), y = count, color = d7_roto_boost, group = d7_roto_boost)) + 
  geom_point() + geom_smooth(se = FALSE, method = "loess") + scale_y_log10()

```
**Interpretation:** There are no differentially abundant taxa in any of the randomization arms at any time point between samples that boosted and those that did not boost.
...
**SHEDDING: BROAD SPECTRUM ANTIBIOTICS SAMPLES**
```{r diff-abund-shedding-broad-LRT}
# Test for taxa which at one or more time points after time 0 showed a shedding-specific effect
# Convert phyloseq object to DeSeq2 table
ds.shedding.broad.LRT <- phyloseq_to_deseq2(ps1.broad, ~shedding + day + shedding:day)

# Calculate geometric means prior to estimate size factors
geoMeans.ds.shedding.broad.LRT <- apply(counts(ds.shedding.broad.LRT), 1, gm_mean)
ds.shedding.broad.LRT <- estimateSizeFactors(ds.shedding.broad.LRT, geoMeans = geoMeans.ds.shedding.broad.LRT)

# Run DeSeq2
dds.shedding.broad.LRT <- DESeq(ds.shedding.broad.LRT, test="LRT", reduced = ~shedding + day)

# Tabulate results
res.dds.shedding.broad.LRT = results(dds.shedding.broad.LRT)
res.dds.shedding.broad.LRT$symbol <- mcols(dds.shedding.broad.LRT)$symbol
head(res.dds.shedding.broad.LRT[order(res.dds.shedding.broad.LRT$padj), ], 4)
write.table(res.dds.shedding.broad.LRT, file = "./results/deseq_shedding_braod_LRT.txt", sep = "\t")
mcols(res.dds.shedding.broad.LRT)
summary(res.dds.shedding.broad.LRT)

# Plot taxa with lowest p-value
fiss.dds.shedding.broad.LRT <- plotCounts(dds.shedding.broad.LRT, which.min(res.dds.shedding.broad.LRT$padj), 
                   intgroup = c("day","d7_roto_boost"), returnData = TRUE)
ggplot(fiss.dds.shedding.broad.LRT,
  aes(x = as.numeric(day), y = count, color = d7_roto_boost, group = d7_roto_boost)) + 
  geom_point() + geom_smooth(se = FALSE, method = "loess") + scale_y_log10()

```

**Interpretation:** There are no differentially abundant taxa in any of the randomization arms at any time point between samples that shed rotavirus and those that did not shed
...
# Differential Abundance Testing
ds.d7.con <- phyloseq_to_deseq2(ps1.d7.con, ~d7_roto_boost)
...
# Differential Abundance Testing
ds.d7.narrow <- phyloseq_to_deseq2(ps1.d7.narrow, ~d7_roto_boost)
...
# Differential Abundance Testing
ds.d7.broad <- phyloseq_to_deseq2(ps1.d7.broad, ~d7_roto_boost)
...
# Differential Abundance Testing
ds.d0.con <- phyloseq_to_deseq2(ps1.d0.con, ~d7_roto_boost)
...
# Differential Abundance Testing
ds.d0.narrow <- phyloseq_to_deseq2(ps1.d0.narrow, ~d7_roto_boost)
...
# Differential Abundance Testing
ds.d0.broad <- phyloseq_to_deseq2(ps1.d0.broad, ~d7_roto_boost)
...
# Differential Abundance Testing
ds.d.9.con <- phyloseq_to_deseq2(ps1.d.9.con, ~d7_roto_boost)
...
# Differential Abundance Testing
ds.d.9.narrow <- phyloseq_to_deseq2(ps1.d.9.narrow, ~d7_roto_boost)
...
# Differential Abundance Testing
ds.d.9.broad <- phyloseq_to_deseq2(ps1.d.9.broad, ~d7_roto_boost)
...
sample_data(ps1.d7.boost_narrow_v_broad)$d7_roto_boost
